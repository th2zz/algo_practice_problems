class Solution0:
    ...
    def strStr2(self, source: str, target: str):
        ...
        for i in range(n):
            ...
            if i < m - 1:
                ...
                i >= m
            ):  # 判断是不是第二个window (第一个window 0...m-1) i在m-1 只有第二个window才需要开始考虑删字符 删掉之前window第一个字符
                # abcd -a 目标找到bcd m=3 power=31^3;
                #    i
                # i = 3, 上面已经*31 +d hashcode = a*31^3 + b*31^2 + c*31 + d, power = 31^3,  source[i - m] = a
                # each iteration the char out of window is guranteed to have <ord of c> * 31^m term, this step remove the highest degree term, next iteration beginning we rolling multiply to make a new highest degree term 31^m
                hash_code = (hash_code - ord(source[i - m]) * power) % modulus
                if hash_code < 0:
                    hash_code += modulus
            if (
                hash_code == target_code
            ):  # hash is the same, can still have collision, verify if actually found
    ...
    def stringReplace(self, a: list[str], b: list[str], s: str):
        ...
        # 查看每个a中pattern长度滑动窗口 通过s前缀hash ahash直接计算此窗口子串hash来比较确定是否match  子串hash=ahash =匹配
        # 这样while内匹配直接变成O(1)
        if not a or not b:
            return s
        # sort = inplace, sorted = new arr
        curr_index = 0  # keep track of progress in s
        end_res = ""
        used = [False] * len(a)
...
class Solution:
    ...
    def lcg(
        self, modulus: int, a: int, c: int, seed: int
    ...
    def stringReplace(self, a, b, s):
        ...
        radix, modulus = (
            31,
            1000000007,
        )  # modulus is hash size  # TODO modulus不能太小 否则会哈希冲突 导致过不了
        ...
        for pattern in a:
            ...
            for c in pattern:  # 每轮c都不一样 这里可以不用generator 对于无重复的str 每个generator实际上只next了一次
                pattern_hash = (
                    (pattern_hash * radix + ord(c)) % modulus
                )  # pattern_hash = next(self.lcg(modulus=modulus, a=pattern_hash, c=curr_char_unicode_codepoint, seed=radix))
            a_hash.append(pattern_hash)
        # 2. precompute prefix hash & base:   s[i]=前i个字符前缀substr hash,  base_arr[i]=seed^i
        s_prefix_hash, base_arr, prefix_hash, base = (
            [0],
            [1],
            0,
            1,
        )  # TODO hash 基础值0 base1 因为以append方式往里加 所以需要初始化
        for c in s:
            prefix_hash = (
                (prefix_hash * radix + ord(c)) % modulus
            )  # prefix_hash = next(self.lcg(modulus=modulus, a=prefix_hash, c=curr_char_unicode_codepoint, seed=radix))
            s_prefix_hash.append(prefix_hash)
            base = (
                (base * radix) % modulus
            )  # base = next(self.lcg(modulus=modulus, a=base, c=0, seed=radix))  # 对应当前prefix str的最高次base
        ...
        while curr_idx < len(s):  # while没有构建完成replaced str
            ...
            for j, pattern in enumerate(a):  # 遍历pattern in a
                if (
                    len(pattern) + curr_idx > len(s)
                ):  # TODO 跳过太长的pattern 例如 curr_idx = 0, len(s) = 4 最多考虑长度为4的pattern
                    continue  # skip too long pattern,
                len_pattern_window_hash = (
                    (
                        s_prefix_hash[curr_idx + len(pattern)]
                        - base_arr[len(pattern)] * s_prefix_hash[curr_idx]
                    )
                    % modulus
                )  # 通过预计算的前缀hash 做差法 计算从curr_idx开始 长度为此pattern长度的窗口 字符串的hash
                len_pattern_window_hash = (len_pattern_window_hash + modulus) % modulus
                if a_hash[j] % modulus == len_pattern_window_hash and max_len < len(
                    pattern
                ):  # 打擂台记录匹配成功最长pattern长度
                    max_len = len(pattern)
                    matched_max_len_pattern_idx = j
            if (
                matched_max_len_pattern_idx == -1
            ):  # 没有找到匹配成功的，保留s原串的当前字母 指针+1
