class Solution:
    ...
    def RotateString2(self, str, left, right):
        total_offset = (left - right) % len(
            str
        )  # 需要注意mod! 否则大数left,right无法处理
        # -1 mod 7 = 6
...
print(Solution().RotateString2(str="abcdefg", left=3, right=1))

print(Solution().RotateString2(str="abcdefg", left=1, right=2))
