class Solution:
    ...
    def rerange(self, A):
        ...
        neg_cnt = self.partition(
            A, 0
        )  # left 是>= 0 的开始 but no zero in input so it's index of first pos num
        pos_cnt = len(A) - neg_cnt
        left = (
            1 if neg_cnt > pos_cnt else 0
        )  # 根据奇偶情况做一个同向双指针interleave TODO 保证两边开始位置 是等距的
    ...
    def interleave(
        self, A, left, right
    ):  # TODO interleave做的就算根据上面错开的left, right 相向双指针 swap后 同步+2-2
        while left < right:
            A[left], A[right] = A[right], A[left]
            left += 2
            right -= 2
