...
class Solution:
    ...
    def build_graph(self, seqs):  # adjacency list
        ...
        for seq in (
            seqs
        ):  # 每一个seq都代表了一系列边的定义 将这些边还原到图里 e.g. [1,2] => 1->2
            for i in range(
                1,
                len(seq),  # traverse num (node) in each seq from the second node
            ):  # graph[from_node].add(to_node) from_node index: [0...len(seq)-2] to_node index: [1...len(seq)-1]
                graph[seq[i - 1]].add(seq[i])  # add edge from prev node to cur node
        return graph
    ...
    def topo_sort(self, graph):
        ...
        while q:
            if (
                len(q) > 1
            ):  # unique topo order, no many to one relations, q length = 1 at all times
                return None
            n = q.popleft()
            topo_order.append(n)
