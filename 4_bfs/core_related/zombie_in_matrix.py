...
class Solution:
    ...
    def zombie(self, grid: List[List[int]]) -> int:
        ...
        for i in range(
            n
        ):  # 初始化: 数下zombie个数 wall个数 把zombie坐标和steps 加到队列作为bfs源点
            for j in range(m):
                if grid[i][j] == 1:
                    q.append(
                        (i, j, 0)
                    )  # node structure: x, y, how many steps from source
                    sum_zombies += 1
                if grid[i][j] == 2:
                    sum_walls += 1
        steps = 0
        while q:
            ...
            for dx, dy in [
                ...
                [0, 1],
                [0, -1],
            ]:  # for each one around zombie
                nx, ny = x + dx, y + dy
                if self.is_valid(
                    nx, ny, grid
                ):  # if it is a valid in-bound person, append to queue and mark as zombie
        ...
        #         if grid[i][j] == 0:
        #             return -1
        return steps  # how many steps to infect all persons
