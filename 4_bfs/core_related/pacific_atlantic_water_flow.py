class Solution:  # https://leetcode.cn/problems/pacific-atlantic-water-flow/
    ...
    def pacificAtlantic(self, heights: List[List[int]]) -> List[List[int]]:
        ...
        # reverse bfs (from destination pacific and atlantic cells to start (higher heights cells within same connected component))
        pacific = [(0, i) for i in range(n)] + [(i, 0) for i in range(1, m)]
        atlantic = [(m - 1, i) for i in range(n)] + [(i, n - 1) for i in range(m - 1)]
        # pacific & atlantic is set of tuple
        # map(list, set(tuple)) => iterable of list  { (1, 2), (3, 4) } =>  [1,2],[3,4]
        return list(
            map(list, bfs(pacific) & bfs(atlantic))
        )  # return pts that can reach both ocean
