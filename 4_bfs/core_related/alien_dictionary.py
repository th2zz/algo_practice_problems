...
class Solution:  # https://leetcode.cn/problems/alien-dictionary/submissions/
    ...
    def build_graph(self, words):  # 本体的难点在于构图
        graph = {}  # possible char to set map
        for seq in words:
            for char in seq:
        ...
        n = len(words)
        for i in range(n - 1):  # traverse pair of words at a time 所以i最大 n-2
            for j in range(
                min(len(words[i]), len(words[i + 1]))
            ):  # based on min length of current word and next word, traverse char by j
                if words[i][j] != words[i + 1][j]:
                    graph[words[i][j]].add(
                        words[i + 1][j]
                    )  # 不等的字符直接 代表 一条边 because words are sorted in lexicographical order
                    break
    ...
    def topo_sort(self, graph):
        ...
        order = ""
        # graph is dict[str, set(str)]  # char to adj set of char map
        # each neighbor is of str type (char) so we need heap (pq)
