...
class BSTIterator:
    ...
    for each node processed make sure to push its right child leftmost branch onto stack
    """
    ...
    def _next(self):  # 这个模板 镜像修改后可以做prev访问
        ...
        if (
            node.right
        ):  # 有右边节点 next = 右子树最小值  把右子树左侧入栈 待下轮next使用
            self.push_leftmost_branch(node.right)
        else:  # 无右边节点 next = 路径上第一个可以与 下一个节点构成左子树关系的节点  对如图上的1
            n = (
                self.stack.pop()
            )  # pop掉栈上所有右子树的部分 包括右子树根节点 栈顶为下轮next的值
